...
begin
  ...
  loop
    ...
    if (kid->>'birthYear')::integer not between 1920 and extract(year from current_date)::integer then raise exception 'Each kid needs a valid birth year'; end if;
    if (kid->>'avatar') not in ('fox', 'bear', 'bunny', 'lion', 'panda', 'frog', 'koala', 'cat', 'dog', 'owl', 'unicorn', 'dino') then raise exception 'Each kid needs a valid avatar'; end if;
    ...
    if existing_kid_id is not null then
      ...
      insert into public.template_collections (user_id, name) values (auth.uid(), collection_name)
  ...
  on conflict (user_id) do update set parent_name = excluded.parent_name, onboarding_completed_at = excluded.onboarding_completed_at;
end;
$$;

grant execute on function public.complete_family_onboarding(text, jsonb) to authenticated;

-- Remove only empty, unlinked duplicates produced by the earlier save bug.
-- Collections containing saved crafts are retained so no user data is lost.
delete from public.template_collections collection
where not exists (select 1 from public.kids where kids.collection_id = collection.id)
  and not exists (select 1 from public.template_collection_items where template_collection_items.collection_id = collection.id)
  and exists (
    select 1 from public.kids
    where kids.user_id = collection.user_id
      and (
        lower(collection.name) = lower('For ' || kids.name)
        or (
          starts_with(lower(collection.name), lower(left('For ' || kids.name, 55) || ' ('))
          and right(collection.name, 1) = ')'
        )
      )
  );
