alter table public.kids
  add column gender text check (gender is null or gender in ('boy', 'girl'));
...
begin
  ...
  loop
    ...
    kid_gender := nullif(kid->>'gender', '');
    if kid_gender is not null and kid_gender not in ('boy', 'girl') then
      raise exception 'Each kid needs a valid gender';
    end if;
    ...
    if existing_kid_id is not null then
      ...
      update public.kids set name = kid_name, birth_year = (kid->>'birthYear')::integer,
        avatar = kid->>'avatar', gender = kid_gender, sort_order = (kid->>'sortOrder')::smallint
      ...
      insert into public.kids (user_id, name, birth_year, avatar, gender, collection_id, sort_order)
      values (auth.uid(), kid_name, (kid->>'birthYear')::integer, kid->>'avatar', kid_gender, new_collection_id, (kid->>'sortOrder')::smallint)
